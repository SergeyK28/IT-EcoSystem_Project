# -*- coding: utf-8 -*-
"""
Модуль для управления складом в CRM (упрощённый дизайн)
"""
...
class WarehouseDialog(QDialog):
    """
    Диалоговое окно для управления складом (упрощённый дизайн)
    """
    ...
    def __init__(self, parent=None):
        ...
        # Базовый тёмный стиль (без излишеств)
    ...
    def setup_ui(self):
        ...
        self.stats_widgets = []
        stats_config = [
            ("📊 Всего позиций", "0", "#2d7d3a"),
            ("🏷️ Категорий", "0", "#e0a800"),
            ("📦 Единиц товара", "0", "#1a7a8a"),
        ...
        apply_filter_btn.setStyleSheet("background-color: #2d7d3a; color: white; font-weight: bold; border: none;")
    ...
    def setup_analytics_tab(self):
        ...
        metrics = [
            ("Общая стоимость запасов:", "0 ₽", "#2d7d3a"),
            ("Потенциальная выручка:", "0 ₽", "#e0a800"),
            ("Потенциальная прибыль:", "0 ₽", "#1a7a8a"),
        ...
        self.top_categories_table.setHorizontalHeaderLabels(["Категория", "Товаров", "Единиц", "Стоимость"])
        ...
        self.top_brands_table.setHorizontalHeaderLabels(["Бренд", "Товаров", "Единиц"])
    ...
    def update_table(self):
        ...
        for row, item in enumerate(self.current_items):
            ...
            if count <= 0:
                count_item.setForeground(QColor(255, 99, 71))
                count_item.setToolTip("Нет в наличии")
            elif count <= min_level:
                count_item.setForeground(QColor(255, 193, 7))
                count_item.setToolTip("Малый запас")
            else:
                count_item.setForeground(QColor(76, 175, 80))
    ...
    def update_statistics(self):
        try:
            ...
            self.stats_widgets[0].findChild(QLabel, "", Qt.FindChildrenRecursively).setText(str(stats.get('total_items', 0)))
            self.stats_widgets[1].findChild(QLabel, "", Qt.FindChildrenRecursively).setText(str(stats.get('total_categories', 0)))
            self.stats_widgets[2].findChild(QLabel, "", Qt.FindChildrenRecursively).setText(str(stats.get('total_units', 0)))
            self.stats_widgets[3].findChild(QLabel, "", Qt.FindChildrenRecursively).setText(f"{stats.get('total_investment', 0):,.2f} ₽".replace(',', ' '))
            self.stats_widgets[4].findChild(QLabel, "", Qt.FindChildrenRecursively).setText(str(stats.get('low_stock_count', 0)))
        except Exception as e:
        ...
        try:
            ...
            for item in self.current_items:
                ...
                if category not in categories:
                    categories[category] = {'count': 0, 'total_units': 0, 'total_value': 0}
            ...
            for row, (category, data) in enumerate(sorted(categories.items())):
                ...
                self.categories_table.setItem(row, 3, self.create_centered_item(f"{data['total_value']:,.2f} ₽".replace(',', ' ')))
        except Exception as e:
        ...
        try:
            ...
            if 'top_categories' in stats:
                ...
                for row, cat in enumerate(stats['top_categories']):
                    ...
                    self.top_categories_table.setItem(row, 3, self.create_centered_item(f"{cat.get('total_value', 0):,.2f} ₽".replace(',', ' ')))
    ...
    def show_item_details(self, row):
        if 0 <= row < len(self.current_items):
            ...
            if details:
                dialog = ItemDetailsDialog(details, self)
                dialog.exec_()
    ...
    def delete_item(self):
        ...
        if result == QMessageBox.Yes:
            try:
                success = db_crm.update_warehouse_item(item.get('StockID'), {**item, 'is_active': False})
                if success:
...
class AddEditItemDialog(QDialog):
    """Диалог добавления/редактирования товара (упрощённый)"""
...
class RestockDialog(QDialog):
    """Диалог пополнения запаса (упрощённый)"""
    ...
    def setup_ui(self):
        ...
        info_layout.addWidget(QLabel(str(self.item_data.get('CountInStock', 0))), 1, 1)
...
class ItemDetailsDialog(QDialog):
    """Диалог с детальной информацией о товаре (упрощённый)"""
