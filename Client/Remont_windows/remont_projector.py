...
class Ui_Ui_Remont_Projector_Dialog(object):
    def setupUi(self, Ui_Remont_Projector_Dialog):
        self.dialog = Ui_Remont_Projector_Dialog

        Ui_Remont_Projector_Dialog.setObjectName("Ui_Remont_Projector_Dialog")
        Ui_Remont_Projector_Dialog.resize(1100, 950)
        Ui_Remont_Projector_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Projector_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Projector_Dialog.setWindowIcon(icon)

        Ui_Remont_Projector_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Projector_Dialog)
        ...
        self.page_title = QLabel("📽️ Ремонт проекторов")
        ...
        self.QW_Remont_projector = QWidget()
        self.QW_Remont_projector.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_projector)
        ...
        self.LB1_Remont_projector = QLabel()
        self.LB1_Remont_projector.setWordWrap(True)
        self.LB1_Remont_projector.setOpenExternalLinks(True)
        self.LB1_Remont_projector.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_projector)
        ...
        table_header = QLabel("💰 Стоимость ремонта проекторов")
        ...
        services_list = [
            {"name": "Замена лампы", "price": "от 1500 ₽"},
            {"name": "Чистка оптики", "price": "от 1200 ₽"},
            {"name": "Ремонт блока питания", "price": "от 1500 ₽"},
            {"name": "Замена матрицы", "price": "от 2000 ₽"},
            {"name": "Ремонт системы охлаждения", "price": "от 1000 ₽"},
            {"name": "Замена поляризатора", "price": "от 1800 ₽"},
            {"name": "Ремонт цветового колеса", "price": "от 1300 ₽"},
            {"name": "Калибровка проектора", "price": "от 800 ₽"}
        ]
        ...
        self.order_button = ModernButton("📽️ Заказать ремонт проектора")
        ...
        self.LB2_Remont_projector = QLabel()
        self.LB2_Remont_projector.setWordWrap(True)
        self.LB2_Remont_projector.setOpenExternalLinks(True)
        self.LB2_Remont_projector.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_projector)
        ...
        self.scrollArea.setWidget(self.QW_Remont_projector)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Projector_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Projector_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Projector_Dialog)

    def retranslateUi(self, Ui_Remont_Projector_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Projector_Dialog.setWindowTitle(_translate("Ui_Remont_Projector_Dialog", "IT-EcoSystem - Ремонт проекторов"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>📽️ Ремонт проекторов любой сложности</h1>
            ...
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    ...
                    <span style='color: #b0b0b0;'>Ремонтируем DLP, LCD, LED проекторы, ультракороткофокусные модели</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> Epson, BenQ, Optoma, ViewSonic, Acer, Sony, Panasonic и другие бренды
            </p>
        </div>
        """
        self.LB1_Remont_projector.setText(_translate("Ui_Remont_Projector_Dialog", lb1_text))
        ...
        <div>
            ...
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>💡 Оптический блок</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Чистка и юстировка оптики, замена поляризаторов, ремонт цветового колеса</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔌 Электроника</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт материнских плат, блоков питания, замена ламп и светодиодов</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🌀 Система охлаждения</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Замена вентиляторов, чистка радиаторов, восстановление терморегуляции</p>
                </div>
            </div>
        ...
        """
        self.LB2_Remont_projector.setText(_translate("Ui_Remont_Projector_Dialog", lb2_text))
...
if __name__ == "__main__":
    ...
    ui = Ui_Ui_Remont_Projector_Dialog()
    ui.setupUi(Dialog)
    Dialog.show()
    sys.exit(app.exec_())