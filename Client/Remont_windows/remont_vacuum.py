...
class Ui_Ui_Remont_Vacuum_Dialog(object):
    def setupUi(self, Ui_Remont_Vacuum_Dialog):
        self.dialog = Ui_Remont_Vacuum_Dialog

        Ui_Remont_Vacuum_Dialog.setObjectName("Ui_Remont_Vacuum_Dialog")
        Ui_Remont_Vacuum_Dialog.resize(1100, 950)
        Ui_Remont_Vacuum_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Vacuum_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Vacuum_Dialog.setWindowIcon(icon)

        Ui_Remont_Vacuum_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Vacuum_Dialog)
        ...
        self.page_title = QLabel("🧹 Ремонт пылесосов")
        ...
        self.QW_Remont_vacuum = QWidget()
        self.QW_Remont_vacuum.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_vacuum)
        ...
        self.LB1_Remont_vacuum = QLabel()
        self.LB1_Remont_vacuum.setWordWrap(True)
        self.LB1_Remont_vacuum.setOpenExternalLinks(True)
        self.LB1_Remont_vacuum.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_vacuum)
        ...
        table_header = QLabel("💰 Стоимость ремонта пылесосов")
        ...
        services_list = [
            {"name": "Замена двигателя", "price": "от 1500 ₽"},
            {"name": "Ремонт шланга", "price": "от 500 ₽"},
            {"name": "Замена аккумулятора (робот-пылесос)", "price": "от 1200 ₽"},
            {"name": "Ремонт турбощетки", "price": "от 800 ₽"},
            {"name": "Замена фильтров", "price": "от 300 ₽"},
            {"name": "Ремонт электроники", "price": "от 1000 ₽"},
            {"name": "Замена колес (робот-пылесос)", "price": "от 900 ₽"},
            {"name": "Диагностика и чистка", "price": "от 500 ₽"}
        ]
        ...
        self.order_button = ModernButton("🧹 Заказать ремонт пылесоса")
        ...
        self.LB2_Remont_vacuum = QLabel()
        self.LB2_Remont_vacuum.setWordWrap(True)
        self.LB2_Remont_vacuum.setOpenExternalLinks(True)
        self.LB2_Remont_vacuum.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_vacuum)
        ...
        self.scrollArea.setWidget(self.QW_Remont_vacuum)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Vacuum_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Vacuum_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Vacuum_Dialog)

    def retranslateUi(self, Ui_Remont_Vacuum_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Vacuum_Dialog.setWindowTitle(_translate("Ui_Remont_Vacuum_Dialog", "IT-EcoSystem - Ремонт пылесосов"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>🧹 Ремонт пылесосов любой сложности</h1>
            <p style='font-size: 16px; margin-bottom: 20px; color: #e0e0e0;'>
                ...
                пылесосов всех типов: традиционных, вертикальных и роботов-пылесосов.
            </p>
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    <span style='color: #4CAF50; font-weight: bold;'>✓ Ремонт любых пылесосов</span><br>
                    <span style='color: #b0b0b0;'>Ремонтируем пылесосы всех брендов и типов</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> Karcher, Samsung, LG, Philips, Bosch, Thomas, Dyson, iRobot, Xiaomi
            </p>
        </div>
        """
        self.LB1_Remont_vacuum.setText(_translate("Ui_Remont_Vacuum_Dialog", lb1_text))
        ...
        <div>
            ...
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔧 Электрическая часть</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт двигателя, замена конденсаторов, восстановление плат управления</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🤖 Роботы-пылесосы</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Замена аккумуляторов, колес, датчиков, ремонт навигации</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔌 Механика</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт шлангов, щеток, турбощеток, восстановление корпусных деталей</p>
                </div>
            </div>
        ...
        """
        self.LB2_Remont_vacuum.setText(_translate("Ui_Remont_Vacuum_Dialog", lb2_text))
