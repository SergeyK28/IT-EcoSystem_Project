...
class Ui_Ui_Remont_Columns_Dialog(object):
    def setupUi(self, Ui_Remont_Columns_Dialog):
        self.dialog = Ui_Remont_Columns_Dialog

        Ui_Remont_Columns_Dialog.setObjectName("Ui_Remont_Columns_Dialog")
        Ui_Remont_Columns_Dialog.resize(1100, 950)
        Ui_Remont_Columns_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Columns_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Columns_Dialog.setWindowIcon(icon)

        Ui_Remont_Columns_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Columns_Dialog)
        ...
        self.page_title = QLabel("🎵 Ремонт колонок")
        ...
        self.page_subtitle = QLabel("Профессиональный ремонт колонок")
        ...
        self.QW_Remont_columns = QWidget()
        self.QW_Remont_columns.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_columns)
        ...
        self.LB1_Remont_columns = QLabel()
        self.LB1_Remont_columns.setWordWrap(True)
        self.LB1_Remont_columns.setOpenExternalLinks(True)
        self.LB1_Remont_columns.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_columns)
        ...
        table_header = QLabel("💰 Стоимость ремонта колонок")
        ...
        services_list = [
            ...
            {"name": "Замена диффузора", "price": "от 1000 ₽"},
            {"name": "Ремонт звуковой катушки", "price": "от 1200 ₽"},
            {"name": "Замена подвеса", "price": "от 600 ₽"},
            {"name": "Ремонт корпуса", "price": "от 800 ₽"},
            {"name": "Замена разъемов", "price": "от 400 ₽"},
        ...
        self.order_button = ModernButton("🎵 Заказать ремонт колонок")
        ...
        self.LB2_Remont_columns = QLabel()
        self.LB2_Remont_columns.setWordWrap(True)
        self.LB2_Remont_columns.setOpenExternalLinks(True)
        self.LB2_Remont_columns.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_columns)
        ...
        self.scrollArea.setWidget(self.QW_Remont_columns)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Columns_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Columns_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Columns_Dialog)

    def retranslateUi(self, Ui_Remont_Columns_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Columns_Dialog.setWindowTitle(_translate("Ui_Remont_Columns_Dialog", "IT-EcoSystem - Ремонт колонок"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>🎵 Ремонт колонок любой сложности</h1>
            <p style='font-size: 16px; margin-bottom: 20px; color: #e0e0e0;'>
                ...
                колонок всех типов: портативных, стационарных и студийных.
            </p>
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    ...
                    <span style='color: #b0b0b0;'>Ремонтируем колонки всех брендов</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> JBL, Sony, Marshall, Bose, Harman Kardon, Sven, Microlab, Logitech
            </p>
        </div>
        """
        self.LB1_Remont_columns.setText(_translate("Ui_Remont_Columns_Dialog", lb1_text))
        ...
        <div>
            ...
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔊 Портативные колонки</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт аккумулятора, замена динамиков, восстановление Bluetooth модуля</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🏠 Стационарные колонки</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт динамиков, замена кроссоверов, восстановление корпуса</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🎧 Студийные мониторы</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Точная настройка, замена высокочастотных и низкочастотных динамиков</p>
                </div>
            </div>
        ...
        """
        self.LB2_Remont_columns.setText(_translate("Ui_Remont_Columns_Dialog", lb2_text))
...
if __name__ == "__main__":
    ...
    ui = Ui_Ui_Remont_Columns_Dialog()
    ui.setupUi(Dialog)
    Dialog.show()
    sys.exit(app.exec_())