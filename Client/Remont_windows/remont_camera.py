...
class Ui_Ui_Remont_Camera_Dialog(object):
    def setupUi(self, Ui_Remont_Camera_Dialog):
        self.dialog = Ui_Remont_Camera_Dialog

        Ui_Remont_Camera_Dialog.setObjectName("Ui_Remont_Camera_Dialog")
        Ui_Remont_Camera_Dialog.resize(1100, 950)
        Ui_Remont_Camera_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Camera_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Camera_Dialog.setWindowIcon(icon)

        Ui_Remont_Camera_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Camera_Dialog)
        ...
        self.page_title = QLabel("📸 Ремонт фотоаппаратов")
        ...
        self.QW_Remont_camera = QWidget()
        self.QW_Remont_camera.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_camera)
        ...
        self.LB1_Remont_camera = QLabel()
        self.LB1_Remont_camera.setWordWrap(True)
        self.LB1_Remont_camera.setOpenExternalLinks(True)
        self.LB1_Remont_camera.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_camera)
        ...
        table_header = QLabel("💰 Стоимость ремонта фотоаппаратов")
        ...
        services_list = [
            {"name": "Чистка матрицы", "price": "от 800 ₽"},
            {"name": "Ремонт затвора", "price": "от 1500 ₽"},
            {"name": "Замена объектива", "price": "от 2000 ₽"},
            {"name": "Ремонт электроники", "price": "от 1200 ₽"},
            {"name": "Калибровка автофокуса", "price": "от 1000 ₽"},
            {"name": "Замена дисплея", "price": "от 1300 ₽"},
            {"name": "Ремонт вспышки", "price": "от 900 ₽"},
            {"name": "Прошивка ПО", "price": "от 800 ₽"}
        ]
        ...
        self.order_button = ModernButton("📸 Заказать ремонт фотоаппарата")
        ...
        self.LB2_Remont_camera = QLabel()
        self.LB2_Remont_camera.setWordWrap(True)
        self.LB2_Remont_camera.setOpenExternalLinks(True)
        self.LB2_Remont_camera.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_camera)
        ...
        self.scrollArea.setWidget(self.QW_Remont_camera)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Camera_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Camera_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Camera_Dialog)

    def retranslateUi(self, Ui_Remont_Camera_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Camera_Dialog.setWindowTitle(_translate("Ui_Remont_Camera_Dialog", "IT-EcoSystem - Ремонт фотоаппаратов"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>📸 Ремонт фотоаппаратов любой сложности</h1>
            <p style='font-size: 16px; margin-bottom: 20px; color: #e0e0e0;'>
                ...
                фотоаппаратов всех марок и моделей.
            </p>
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    <span style='color: #4CAF50; font-weight: bold;'>✓ Профессиональное оборудование</span><br>
                    <span style='color: #b0b0b0;'>Ремонтируем Canon, Nikon, Sony, Fujifilm, Panasonic и другие бренды</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> Canon, Nikon, Sony, Fujifilm, Panasonic, Olympus, Leica и другие бренды
            </p>
        </div>
        """
        self.LB1_Remont_camera.setText(_translate("Ui_Remont_Camera_Dialog", lb1_text))

        lb2_text = """
        <div>
            <h2 style='color: #4CAF50; font-size: 26px; margin-bottom: 25px; border-bottom: 2px solid #4CAF50; padding-bottom: 10px;'>
                🔧 Виды ремонта фотоаппаратов
            </h2>
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔧 Механический ремонт</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт затвора, диафрагмы, механизма зума, байонета объектива</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔌 Электронный ремонт</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт материнских плат, замена сенсоров, восстановление электроники</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>📱 Оптический ремонт</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Чистка оптики, юстировка линз, ремонт стабилизатора изображения</p>
                </div>
            </div>
        ...
        """
        self.LB2_Remont_camera.setText(_translate("Ui_Remont_Camera_Dialog", lb2_text))
