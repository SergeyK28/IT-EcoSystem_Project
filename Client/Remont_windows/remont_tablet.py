...
class Ui_Ui_Remont_Tablet_Dialog(object):
    def setupUi(self, Ui_Remont_Tablet_Dialog):
        self.dialog = Ui_Remont_Tablet_Dialog

        Ui_Remont_Tablet_Dialog.setObjectName("Ui_Remont_Tablet_Dialog")
        Ui_Remont_Tablet_Dialog.resize(1100, 950)
        Ui_Remont_Tablet_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Tablet_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Tablet_Dialog.setWindowIcon(icon)

        Ui_Remont_Tablet_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Tablet_Dialog)
        ...
        self.page_title = QLabel("📟 Ремонт планшетов")
        ...
        self.QW_Remont_tablet = QWidget()
        self.QW_Remont_tablet.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_tablet)
        ...
        self.LB1_Remont_tablet = QLabel()
        self.LB1_Remont_tablet.setWordWrap(True)
        self.LB1_Remont_tablet.setOpenExternalLinks(True)
        self.LB1_Remont_tablet.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_tablet)
        ...
        table_header = QLabel("💰 Стоимость ремонта планшетов")
        ...
        services_list = [
            {"name": "Замена дисплея (экран/тачскрин)", "price": "от 1500 ₽"},
            {"name": "Замена аккумулятора", "price": "от 1200 ₽"},
            {"name": "Замена разъема зарядки", "price": "от 900 ₽"},
            {"name": "Ремонт материнской платы", "price": "от 1500 ₽"},
            {"name": "Восстановление после залития", "price": "от 1800 ₽"},
            {"name": "Замена кнопок и шлейфов", "price": "от 800 ₽"},
            {"name": "Прошивка/переустановка ПО", "price": "от 1000 ₽"},
            {"name": "Замена корпуса", "price": "от 1300 ₽"}
        ]
        ...
        self.order_button = ModernButton("📟 Заказать ремонт планшета")
        ...
        self.LB2_Remont_tablet = QLabel()
        self.LB2_Remont_tablet.setWordWrap(True)
        self.LB2_Remont_tablet.setOpenExternalLinks(True)
        self.LB2_Remont_tablet.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_tablet)
        ...
        self.scrollArea.setWidget(self.QW_Remont_tablet)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Tablet_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Tablet_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Tablet_Dialog)

    def retranslateUi(self, Ui_Remont_Tablet_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Tablet_Dialog.setWindowTitle(_translate("Ui_Remont_Tablet_Dialog", "IT-EcoSystem - Ремонт планшетов"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>📟 Ремонт планшетов любой сложности</h1>
            <p style='font-size: 16px; margin-bottom: 20px; color: #e0e0e0;'>
                ...
                планшетов всех марок и моделей.
            </p>
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    ...
                    <span style='color: #b0b0b0;'>Специалисты с многолетним опытом работы с планшетами Apple iPad, Samsung, Huawei, Lenovo и других брендов</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> Apple iPad, Samsung Galaxy Tab, Huawei, Lenovo, Xiaomi, Sony и другие бренды
            </p>
        </div>
        """
        self.LB1_Remont_tablet.setText(_translate("Ui_Remont_Tablet_Dialog", lb1_text))

        lb2_text = """
        <div>
            <h2 style='color: #4CAF50; font-size: 26px; margin-bottom: 25px; border-bottom: 2px solid #4CAF50; padding-bottom: 10px;'>
                🔧 Почему стоит доверить ремонт планшета профессионалам?
            </h2>
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>✅ Качественные комплектующие</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Используем только оригинальные запчасти и расходные материалы</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>⚡ Скорость выполнения</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Большинство ремонтов выполняются в день обращения</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🔧 Современное оборудование</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Используем профессиональное диагностическое и ремонтное оборудование</p>
                </div>
            </div>
        ...
        """
        self.LB2_Remont_tablet.setText(_translate("Ui_Remont_Tablet_Dialog", lb2_text))
