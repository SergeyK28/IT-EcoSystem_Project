...
class Ui_Ui_Remont_Fan_Dialog(object):
    def setupUi(self, Ui_Remont_Fan_Dialog):
        self.dialog = Ui_Remont_Fan_Dialog

        Ui_Remont_Fan_Dialog.setObjectName("Ui_Remont_Fan_Dialog")
        Ui_Remont_Fan_Dialog.resize(1100, 950)
        Ui_Remont_Fan_Dialog.setMinimumSize(1000, 800)

        font = QFont("Segoe UI", 10)
        Ui_Remont_Fan_Dialog.setFont(font)

        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap("../Pictures/Screenshot from 2025-09-15 14-30-16.png"),
                       QtGui.QIcon.Normal, QtGui.QIcon.Off)
        Ui_Remont_Fan_Dialog.setWindowIcon(icon)

        Ui_Remont_Fan_Dialog.setStyleSheet("""
            QDialog {
        ...
        self.main_layout = QtWidgets.QVBoxLayout(Ui_Remont_Fan_Dialog)
        ...
        self.page_title = QLabel("🌀 Ремонт вентиляторов")
        ...
        self.QW_Remont_fan = QWidget()
        self.QW_Remont_fan.setStyleSheet("background-color: transparent;")
        self.gridLayout = QtWidgets.QGridLayout(self.QW_Remont_fan)
        ...
        self.LB1_Remont_fan = QLabel()
        self.LB1_Remont_fan.setWordWrap(True)
        self.LB1_Remont_fan.setOpenExternalLinks(True)
        self.LB1_Remont_fan.setStyleSheet("""
            QLabel {
                color: #f0f0f0;
        ...
        """)
        info_layout.addWidget(self.LB1_Remont_fan)
        ...
        table_header = QLabel("💰 Стоимость ремонта вентиляторов")
        ...
        services_list = [
            {"name": "Замена двигателя", "price": "от 1000 ₽"},
            {"name": "Ремонт переключателя скоростей", "price": "от 600 ₽"},
            {"name": "Замена подшипников", "price": "от 800 ₽"},
            {"name": "Ремонт блока управления", "price": "от 900 ₽"},
            {"name": "Замена лопастей", "price": "от 500 ₽"},
            {"name": "Ремонт пульта ДУ", "price": "от 500 ₽"},
            {"name": "Чистка и смазка", "price": "от 400 ₽"},
        ...
        self.order_button = ModernButton("🌀 Заказать ремонт вентилятора")
        ...
        self.LB2_Remont_fan = QLabel()
        self.LB2_Remont_fan.setWordWrap(True)
        self.LB2_Remont_fan.setOpenExternalLinks(True)
        self.LB2_Remont_fan.setStyleSheet("""
            QLabel {
                color: #e0e0e0;
                background: transparent;
                font-size: 15px;
        ...
        """)
        detail_layout.addWidget(self.LB2_Remont_fan)
        ...
        self.scrollArea.setWidget(self.QW_Remont_fan)
        self.main_layout.addWidget(self.scrollArea)

        self.back_button.clicked.connect(Ui_Remont_Fan_Dialog.close)

        self.setup_favorites_button()
        self.check_favorite_status()

        self.retranslateUi(Ui_Remont_Fan_Dialog)
        QtCore.QMetaObject.connectSlotsByName(Ui_Remont_Fan_Dialog)

    def retranslateUi(self, Ui_Remont_Fan_Dialog):
        _translate = QtCore.QCoreApplication.translate
        Ui_Remont_Fan_Dialog.setWindowTitle(_translate("Ui_Remont_Fan_Dialog", "IT-EcoSystem - Ремонт вентиляторов"))

        lb1_text = """
        <div style='text-align: left;'>
            <h1 style='color: #4CAF50; font-size: 32px; margin: 0 0 20px 0;'>🌀 Ремонт вентиляторов</h1>
            <p style='font-size: 16px; margin-bottom: 20px; color: #e0e0e0;'>
                ...
                вентиляторов всех типов: напольных, настольных, настенных, потолочных.
            </p>
            <div style='background-color: rgba(76, 175, 80, 0.1); border-left: 4px solid #4CAF50; padding: 15px; margin: 20px 0; border-radius: 0 10px 10px 0;'>
                <p style='margin: 0; font-size: 15px;'>
                    <span style='color: #4CAF50; font-weight: bold;'>✓ Ремонт любых вентиляторов</span><br>
                    <span style='color: #b0b0b0;'>Ремонтируем вентиляторы всех брендов</span>
                </p>
            </div>
            <p style='font-size: 15px; margin: 15px 0; color: #d0d0d0;'>
                <b style='color: #4CAF50;'>Мы ремонтируем:</b> Vitek, Scarlett, Polaris, Electrolux, Bork, Stadler Form и другие
            </p>
        </div>
        """
        self.LB1_Remont_fan.setText(_translate("Ui_Remont_Fan_Dialog", lb1_text))
        ...
        <div>
            ...
            <div style='margin: 20px 0;'>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    ...
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт электродвигателя, замена конденсаторов, восстановление обмоток</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>⚙️ Механика</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Замена подшипников, ремонт механизма поворота, замена лопастей</p>
                </div>
                <div style='margin-bottom: 15px; padding: 20px; background-color: #2d2d2d; border-radius: 15px; border-left: 4px solid #4CAF50;'>
                    <h3 style='color: #4CAF50; margin: 0 0 10px 0;'>🎮 Управление</h3>
                    <p style='color: #b0b0b0; margin: 0;'>Ремонт плат управления, замена переключателей, ремонт пультов ДУ</p>
                </div>
            </div>
            <div style='background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #1a3a1a, stop:1 #2a4a2a); padding: 20px; border-radius: 15px; margin: 30px 0;'>
                ...
                <table width='100%' style='color: #e0e0e0;'>
                    ...
                    <td style='padding: 5px;'>✓ <b style='color: #4CAF50;'>Быстрый ремонт</b> от 1 часа</td>
                    </tr>
        ...
        """
        self.LB2_Remont_fan.setText(_translate("Ui_Remont_Fan_Dialog", lb2_text))
