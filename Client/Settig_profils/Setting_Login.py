...
class Ui_Setting_Login(object):
    def setupUi(self, Setting_Login):
        ...
        Setting_Login.resize(400, 250)
        Setting_Login.setStyleSheet("""
            QDialog {
        ...
        # Заголовок
        self.title_label = QtWidgets.QLabel("🔑 Изменение логина")
        ...
        # Информация
        info_label = QtWidgets.QLabel("Введите новый логин (только латинские буквы и цифры)")
        ...
        self.cancel_button.clicked.connect(Setting_Login.reject)
    ...
    def retranslateUi(self, Setting_Login):
        ...
        Setting_Login.setWindowTitle(_translate("Setting_Login", "IT-EcoSystem - Логин"))

    def save_changes(self):
        new_login = self.login_input.text().strip()

        if not new_login:
            QMessageBox.warning(self.parent(), "Ошибка", "Введите новый логин!")
            return

        if len(new_login) < 3:
            QMessageBox.warning(self.parent(), "Ошибка", "Логин должен содержать минимум 3 символа!")
            return

        if not new_login.isalnum():
            QMessageBox.warning(self.parent(), "Ошибка", "Логин может содержать только буквы и цифры!")
            return

        # Здесь должна быть функция сохранения в БД
        # success = update_login_in_db(self.user_id, new_login)
        success = True  # Заглушка

        if success:
            QMessageBox.information(self.parent(), "Успех", "Логин успешно обновлен!")
            self.parent().accept()
        else:
            QMessageBox.critical(self.parent(), "Ошибка", "Не удалось обновить логин. Возможно, такой логин уже занят.")
